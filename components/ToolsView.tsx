...
import { User, Project, CostEstimate, SafetyIncident, DailyLog } from '../types';
...
const DailyLogSummarizer: React.FC<ToolsViewProps> = ({ user, addToast }) => {
    const [projects, setProjects] = useState<Project[]>([]);
    const [selectedProjectId, setSelectedProjectId] = useState<string>('');
    ...
    const handleGenerate = async () => {
        ...
        try {
            const allLogs = await api.getDailyLogsByProject(parseInt(selectedProjectId, 10));
            const now = new Date();
            const rangeStart = new Date(now);
            rangeStart.setDate(now.getDate() - parseInt(dateRange, 10));
            
            const filteredLogs = allLogs.filter(log => new Date(log.date) >= rangeStart);

            if (filteredLogs.length === 0) {
                setSummary("No logs found in the selected date range to summarize.");
                addToast("No recent logs to summarize.", 'error');
                setIsLoading(false);
                return;
            }

            const result = await api.generateDailyLogSummary(filteredLogs);
            setSummary(result);
            addToast("AI summary generated successfully!", 'success');
        } catch (err) {
            const errorMessage = "Failed to generate AI summary.";
            setError(errorMessage);
            addToast(errorMessage, 'error');
        } finally {
            setIsLoading(false);
    ...
    return (
        <Card>
            <h3 className="text-xl font-semibold text-slate-700 mb-2">AI Daily Log Summary</h3>
            <p className="text-sm text-slate-500 mb-4">Generate a concise, professional report by summarizing daily site logs over a period of time.</p>
            
            <div className="grid grid-cols-1 md:grid-cols-3 gap-4 items-end">
                <div className="md:col-span-2">
                    <label htmlFor="project-select-log" className="block text-sm font-medium text-gray-700 mb-1">Select Project</label>
                    <select
                        id="project-select-log"
                        value={selectedProjectId}
                        onChange={(e) => setSelectedProjectId(e.target.value)}
                        className="w-full p-2 border border-gray-300 rounded-md"
                    >
            ...
            {isLoading && (
                <div className="mt-6 text-center">
                    <p className="text-slate-600 animate-pulse">AI is reading the logs and preparing your summary...</p>
                </div>
            )}
            ...
            {summary && (
                <div className="mt-6 pt-6 border-t">
                    <div className="flex justify-between items-center mb-4">
                        <h4 className="text-lg font-semibold text-slate-800">Generated Summary</h4>
...
export const ToolsView: React.FC<ToolsViewProps> = ({ user, addToast }) => {
    const [activeTool, setActiveTool] = useState<'estimator' | 'scheduler' | 'safety' | 'logSummary'>('estimator');
        ...
        <div>
            ...
            <div className="flex flex-wrap gap-2 mb-4 border-b">
                ...
                >
                 ...
                 <button 
                    onClick={() => setActiveTool('logSummary')}
                    className={`px-4 py-2 text-sm font-medium transition-colors rounded-t-md ${activeTool === 'logSummary' ? 'border-b-2 border-slate-800 text-slate-800' : 'text-slate-500 hover:bg-slate-100'}`}
                >
                    Daily Log Summary
                </button>
            </div>
            ...
            {activeTool === 'logSummary' && <DailyLogSummarizer user={user} addToast={addToast} />}
        </div>
    );
};